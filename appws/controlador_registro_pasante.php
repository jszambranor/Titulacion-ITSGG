<?php
include_once("clases/tb_registro_pasante.php");

class controlador_registro_pasante{
    //Atributo
    private $tb_registro_pasante;
    //Metodos
    public function __construct() {
        $this->tb_registro_pasante = new tb_registro_pasante();
    }
    
    public function index() {
        $resultado = $this->tb_registro_pasante->listar();
        return $resultado;
    }

    public function crear($request,$pst_foto){
        foreach($request as $key => $value){
            $this->tb_registro_pasante->set($key, $value);
        }
        $resultado = $this->tb_registro_pasante->crear();

        
        $this->tb_registro_pasante->set('pst_foto', $pst_foto);
        $resp2 = $this->tb_registro_pasante->gestionfoto($resultado);
        return 'Pasante creado correctamente';
    }

    public function delete($codice){
        $this->tb_registro_pasante->set('pst_id', $codice);
        $this->tb_registro_pasante->eliminar();
    }
