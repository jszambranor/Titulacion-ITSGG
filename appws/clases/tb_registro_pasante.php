...
class tb_registro_pasante {
    ...
    public function listar(){
        $sql= "SELECT * FROM tb_registro_pasante";
        $resultado = $this->con->consultaRetorno($sql);
        return $resultado;
    }

    public function crear(){
        $sql = "INSERT INTO tb_registro_pasante (pst_cedula, pst_nombres, pst_apellidos, pst_convencional, pst_celular, pst_correo, pst_direccion, pst_fecha_nacimiento, pst_edad, pst_enfermedad, pst_alergias, pst_tipo_sangre, pst_genero, pst_curso, pst_jornada, pst_trabaja, pst_provincia, pst_ciudad, pst_dependiente, pst_vive, pst_civil, pst_hijos, pst_universidad, pst_nombre_ref, pst_apellidos_ref, pst_direccion_ref, pst_tlf_ref, pst_aceptar, pst_fecha)
    ...
    */
    ...
    public function editar($campo, $extension, $id){
        $sql = "UPDATE tb_registro_pasante SET ".$campo."='".$extension."' WHERE pst_id = '".$id."'";
        $this->con->consultaSimple($sql);
    }

    public function eliminar() {
        $sql = "DELETE FROM tb_registro_pasante WHERE pst_id = '{$this->pst_id}'";
        $this->con->consultaSimple($sql);
    }

    public function ver () {
        $sql= "SELECT * FROM tb_registro_pasante WHERE pst_id = '{$this->pst_id}' LIMIT 1";
        $resultado = $this->con->consultaRetorno($sql);
        return $resultado;
    }
}
