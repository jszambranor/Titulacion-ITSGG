...
class tb_curriculum {
    ...
    public function crear(){
        $sql = "INSERT INTO tb_curriculum (est_cedula, est_nombres, est_apellidos, est_convencional, est_celular, est_correo, est_direccion, est_fecha_nacimiento, est_edad, est_enfermedad, est_alergias, est_tipo_sangre, est_genero, est_curso,  est_jornada, est_provincia, est_ciudad, est_trabajo, est_vive, est_hermanos, est_civil, est_hijos, est_nombre_ref, est_apellidos_ref, est_cedula_ref, est_tlf_ref, est_correo_ref, est_direccion_ref, est_aceptar, est_fecha)
        VALUES ('{$this->est_cedula}','{$this->est_nombres}','{$this->est_apellidos}','{$this->est_convencional}','{$this->est_celular}','{$this->est_correo}','{$this->est_direccion}','{$this->est_fecha_nacimiento}','{$this->est_edad}','{$this->est_enfermedad}','{$this->est_alergias}','{$this->est_tipo_sangre}','{$this->est_genero}','{$this->est_curso}','{$this->est_jornada}','{$this->est_provincia}','{$this->est_ciudad}','{$this->est_trabajo}','{$this->est_vive}','{$this->est_hermanos}','{$this->est_civil}','{$this->est_hijos}','{$this->est_nombre_ref}','{$this->est_apellidos_ref}','{$this->est_cedula_ref}','{$this->est_tlf_ref}','{$this->est_correo_ref}','{$this->est_direccion_ref}','{$this->est_aceptar}',NOW())";
    ...
    public function actualizar($campo, $extension, $id){
        $sql = "UPDATE tb_curriculum SET ".$campo."='".$extension."' WHERE est_id = '".$id."'";
        $this->con->consultaSimple($sql);
    }

    public function eliminar() {
        $sql = "DELETE FROM tb_curriculum WHERE est_id = '{$this->est_id}'";
        $this->con->consultaSimple($sql);
    }

    public function ver () {
        $sql= "SELECT * FROM tb_curriculum WHERE est_id = '{$this->est_id}' LIMIT 1";
        $resultado = $this->con->consultaRetorno($sql);
        return $resultado;
    }
}
